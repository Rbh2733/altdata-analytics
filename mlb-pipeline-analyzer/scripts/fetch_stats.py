...
Four derived files, all written to data/derived/:
  ...
  season_splits.csv    one row per (player, season, level, team) hitting
                       stat line, minors and MLB
  ...
  league_baselines.csv summed team totals per (level, season), the
...
excluded from the population entirely (see constants.PITCHER_POSITIONS),
and only the hitting stat group is ever fetched. Pitcher fetching
(PIT_FIELDS, groups_for's pitching branch) is preserved at the
`pitcher-work-2026-08-02` git tag, not deleted.

Call budget: roughly one yearByYear call per (player, level), one
teams-stats call per (level, season), one bulk players call per
...
def main():
    ...
    for r in crosswalk:
        ...
        if pid and pid in universe:
            pos = (universe[pid].get("primary_position", "") or "").strip().upper()
            if pos in constants.PITCHER_POSITIONS:
                excluded_pitchers.add(pid)
                continue
            players[pid] = universe[pid]
    print(f"{len(players)} unique matched hitters to fetch "
          f"({len(excluded_pitchers)} unique pure-pitcher-position players excluded)")
    ...
    # League baselines: hitting for every minor (sport, season) pair plus
    # MLB for every season touched.
    baselines = []
    ...
    print(f"baselines: {len(baselines)} (level, season) rows")
